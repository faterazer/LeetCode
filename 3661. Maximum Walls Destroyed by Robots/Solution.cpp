...
public:
    ...
    {
        int n = robots.size();
        vector<pair<int, int>> sorted_robots(n + 2);
        sorted_robots[0] = { 0, 0 };
        for (int i = 0; i < n; ++i)
            sorted_robots[i + 1] = { robots[i], distance[i] };
        sorted_robots[n + 1] = { numeric_limits<int>::max(), 0 };
        ranges::sort(sorted_robots);
        ranges::sort(walls);

        function<int(int, int)> count_walls = [&](int start, int end) -> int {
            if (start > end)
                return 0;
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            int left_limit = max(prev_pos + 1, curr_pos - curr_dist);
            int walls_on_left = count_walls(left_limit, curr_pos);
            int candidate_from_left = dp[0] + walls_on_left;
            int last_right_limit = min(prev_pos + prev_dist, curr_pos - 1);
            int candidate_from_right = dp[1] + walls_on_left - count_walls(left_limit, last_right_limit);

            int right_limit = min(next_pos - 1, curr_pos + curr_dist);
            int walls_on_right = count_walls(curr_pos, right_limit);
            dp[1] = max(dp[0], dp[1]) + walls_on_right;
            dp[0] = max(candidate_from_left, candidate_from_right);
        }
